...
export default function AdminOrdersPage() {
  ...
  const [orderToApprove, setOrderToApprove] = useState<{ id: string; orderNumber: string; total: number } | null>(null);
  ...
  // Open approval modal
  const openApprovalModal = (orderId: string, orderNumber: string, total: number) => {
    setOrderToApprove({ id: orderId, orderNumber, total });
  ...
  const confirmApprovePayment = async () => {
    ...
    try {
      setUpdatingOrderId(orderToApprove.id);
      const response = await fetch('/api/admin/orders', {
        method: 'PATCH',
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {order.paymentStatus === 'PENDING' && !(order.status === 'CANCELLED' || order.cancelledAt) && (
                          <button
                            onClick={() => openApprovalModal(order.id, order.orderNumber, order.total)}
      ...
      {showApprovalModal && orderToApprove && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl max-w-md w-full p-6 animate-in zoom-in-95 duration-200">
            {/* Icon */}
            ...
            <p className="text-center text-gray-600 dark:text-gray-400 mb-6">
              You are about to approve payment for this order. The customer will be notified via email.
            </p>
            ...
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 mb-6 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-semibold text-gray-900 dark:text-white">{orderToApprove.orderNumber}</span>
              </div>
              <div className="flex justify-between items-center pt-3 border-t border-gray-200 dark:border-gray-600">
                <span className="text-sm text-gray-600 dark:text-gray-400">Amount</span>
                <span className="text-lg font-bold text-green-600 dark:text-green-400">
                  ${orderToApprove.total.toFixed(2)}
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={updatingOrderId === orderToApprove.id}
              ...
              <button
                ...
                disabled={updatingOrderId === orderToApprove.id}
                className="flex-1 px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors font-medium disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {updatingOrderId === orderToApprove.id ? (
                  <>
